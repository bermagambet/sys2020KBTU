...
void kfifo_example(void) {
	...
	i = 1;
	while(i < 30){
	struct mytask_struct *maintask2 = kmalloc(sizeof(struct mytask_struct), GFP_KERNEL);
	maintask2->id = i + 1;
	maintask2->priority = (i+1)*2;
	kfifo_in(&queue, maintask2, sizeof(maintask2));
	i+= 1;
	print_task(maintask2);
	}
	...
	{
		qsize = kfifo_len(&queue);
		printk(KERN_NOTICE "Size of queue: %d, Size of task value: %d", qsize, sizeof(&val_mytask);
