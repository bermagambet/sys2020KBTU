...
//allocating enough memory pages for our struct to be fitted right
struct page* mypages;
void pageAllocation(int temp1) {
   ...
   printk(KERN_INFO "2^%d=%d # of pages allocated.", pow, temp2);
   getnstimeofday(&t1);
   mypages = alloc_pages(GFP_KERNEL, pow); //inside the kernel allocation
   __free_pages(mypages, pow);
   getnstimeofday(&t2);
   printk(KERN_INFO "Start: %lu. 
                     Finish: %lu. Throught: %lu.", 
